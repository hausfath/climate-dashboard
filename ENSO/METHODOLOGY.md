...
| **CanSIPS** | Compute per-member tropical-mean SST from the cached GRIB2 (already global 1°). Anomalies are derived using the persisted observed tropical anomaly as the climatology reference, since the GRIB files contain raw SST rather than anomalies. The persisted anchor is `nino34_anom(m) − rnino34(m)/a(m)` for the latest month m present in both observed monthly files — same-month and de-scaled to the bare convention, so the downstream `× a(m)` applies exactly once. (Before 2026-07-31 it subtracted the *scaled, seasonal* RONI from a potentially *later* monthly Niño 3.4, which inflated the tropical baseline by ~0.4 °C during the fast-intensifying 2026 event and biased CanSIPS rONI low by ~0.45 °C at every lead; verified against tropical-box anomalies from the dashboard's OISST daily pipeline.) |
