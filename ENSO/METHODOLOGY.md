...
| **CanSIPS** | Compute per-member tropical-mean SST from the cached GRIB2 (already global 1°). Because Datamart serves raw SST with no hindcast climatology, member tropical anomalies are member deviations from the joint ensemble mean added to an **observation-anchored, teleconnection-evolved ensemble-mean tropical anomaly** (uniquely among sources, which all carry their models' own tropical anomalies). The anchor is `nino34_anom(m) − rnino34(m)/a(m)` for the latest month m present in both observed monthly files — same-month and de-scaled to the bare convention, so the downstream `× a(m)` applies exactly once. The anchor is then evolved across leads as `trop(T) = anchor + 0.192 × (n34(T−2 mo) − n34(anchor−2 mo))`, with the Niño 3.4 trajectory splicing observed months and the run's own CSV ensemble means (slope/lag from OISSTv2.1 monthly 1982–2026, detrended, r = 0.84; the fit validates on July 2026: predicted +0.79 vs observed +0.78). **History:** before 2026-07-31 the reference was a *constant* baseline, and additionally computed as monthly Niño 3.4 minus *scaled seasonal* RONI from a possibly earlier month. During the fast-intensifying 2026 event the convention mixing inflated the anchor by ~0.4 °C (rONI ~0.45 °C low at all leads), and after fixing that, the constant baseline pinned the tropics ~0.3–0.6 °C below what NMME (+1.02) and C3S (+1.16) members forecast for Dec 2026, forcing the two sub-models' tropical anomalies to be mirror images around a constant (CanESM5's implied tropics went negative in mid-2027 and its scaled rONI exceeded its raw Niño 3.4). Both fixed 2026-07-31; verified against the OISST daily tropical box. **Caveat:** the 0.192 slope is a linear fit over observed Niño 3.4 ≤ ~2.6 °C; the 2026 event extrapolates beyond it, and the evolved tropics run ~0.15–0.25 °C warmer at the Dec peak than NMME/C3S members suggest (mild saturation), i.e. CanSIPS rONI may still be biased slightly *low* rather than high. |
...
All ensemble members from all sources (deduplicated, 13 models, ~637 members) in a single plume plot, alongside recent observed ONI for historical context. Members are color-coded by model, with per-model means as thicker lines and the multi-model median (model-weighted) as a dashed black line. Target months with fewer than 4 distinct models reporting are dropped. This is the most comprehensive view of the full forecast distribution.
